...
void ausgeliehendeMedienAusgeben (std::vector<Medium*>& medien);
...
int main()
{
    ...
    do  // Anzeige des Men�s
    {
        std::cout << std::endl
				  ...
				  << "(6): ausgeliehenen Medien ausgeben" <<std::endl
                  << "(7): Beenden" << std::endl;
        ...
        switch (abfrage)
        {
            ...
            case '6':
            {
            	ausgeliehendeMedienAusgeben(medien);
            	break;
            }
            case '7':
            {
...
/**
 * @brief Funktion gibt alle ausgeliehende Median aus
 */
void ausgeliehendeMedienAusgeben (std::vector<Medium*>& medien)
{
	std::cout<<"ausgeliehende Medien:"<<std::endl;
	bool keine = true;

	for(unsigned int i=0; i< medien.size(); i++){
		if(medien[i]->getstatus() == true){
			std::cout << "*************************************************************" << std::endl;
			        medien[i]->ausgabe(std::cout);
			        keine = false;
		}
	}
	if (keine == true){
		std::cout << "Keine Medien wurde zurzeit ausgeliehen" << std::endl;
	}
}
